...
void Window_Init(System_Window Window)
{
    ...
    case System_Window::LORA_TEST:
    {
        ...
        {
            log_printf("S62F (SX1262) init fail, error code: %d\n", state);

            System_Op.init_flag.lora = false;
        }
        else
        {
            log_printf("S62F (SX1262) init successful, frequency: %.03f MHz\n",
                       SX1262_OP.frequency.value);
            ...
            {
                log_printf("S62F failed to set default parameters\n");
                log_printf("S62F assertion: %s\n", buffer_str.c_str());
                System_Op.init_flag.lora = false;
            }
            ...
            if (state != RADIOLIB_ERR_NONE)
            {
                log_printf("S62F failed to start receive, error code: %d\n", state);
                System_Op.init_flag.lora = false;
                break;
            }
...
void Window_End(System_Window Window)
{
    ...
    case System_Window::LORA_TEST:
        ...
        if (System_Op.init_flag.lora == true)
        {
            ...
            if (state != RADIOLIB_ERR_NONE)
            {
                log_printf("S62F sleep fail, error code: %d\n", state);
            }
        }
        Custom_SPI_3.end();
...
void loop()
{
    ...
    case System_Window::GPS_TEST:
        ...
        if (millis() > CycleTime)
        {
            ...
            // Check whether Serial2 has buffered data.
            if (Uart_Rx_Count >= MAX_UART_RX_BUFFER_SIZE)
            {
                Uart_Rx_Buffer[MAX_UART_RX_BUFFER_SIZE - 1] = '\0';

                // Print the relevant RMC information.
                log_printf("---begin---\n%s \n---end---\n", Uart_Rx_Buffer.get());

                log_printf("---RMC---\n");

                // Store the parsed RMC result in a local object.
    ...
    case System_Window::LORA_TEST:
        if (System_Op.init_flag.lora == true)
        {
            ...
            log_printf("[SX1262] Receive event source: ");
            log_printf(dio1Event ? "DIO1\n" : "SPI-poll\n");

            String str;
            const int16_t readState = radio.readData(str);

            if (readState == RADIOLIB_ERR_NONE)
            {
                log_printf("[SX1262] Received packet!\n");
                log_printf("[SX1262] Data:\t\t%s\n", str.c_str());
                ...
                log_printf("[SX1262] RSSI:\t\t%.2f dBm\n", radio.getRSSI());
                log_printf("[SX1262] SNR:\t\t%.2f dB\n", radio.getSNR());
                log_printf("[SX1262] Frequency error:\t%.2f Hz\n",
                           radio.getFrequencyError());
            }
            else if (readState == RADIOLIB_ERR_CRC_MISMATCH)
            {
                log_printf("[SX1262] CRC error!\n");
            }
            else
            {
                log_printf("[SX1262] Receive failed, code %d\n", readState);
            }

            const int16_t receiveState = radio.startReceive();
            if (receiveState != RADIOLIB_ERR_NONE)
            {
                log_printf("[SX1262] Starting to listen failed, code %d\n",
                           receiveState);
                System_Op.init_flag.lora = false;
            }
        }
